...
function random() {
    var review = randomInt(20);
    var galery = _.times(review, function () {
        return faker.image.city(1024, 768)
    });
    var reviews = _.times(review, function (n) {
        return {
            author_name: faker.name.findName(),
            author_image: faker.internet.avatar(),
            date: faker.date.past(),
            rating: randomInt(5),
            review_text: faker.lorem.lines()
        }
    });
...
function infobox() {
  var html = '<div data-id="" class="item infobox">'
             + '<a href="">'
             + '<div class="description"></div>'
             + '<div style="background-image: url('+faker.image.nature()+')" class="image"></div>'
             + '</a>'
