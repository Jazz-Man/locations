...
var addClass = function (el, c) { // where modern browsers fail, use classList
  ...
  isIE = (
    ...
    .exec(navigator.userAgent) != null) ? parseFloat(RegExp.$1) : false,
...
var Affix = function (element, options) {
  ...
  this.options.target = options.target ? (
    (
      typeof (
        options.target) === 'object') ? options.target : document.querySelector(options.target)) : null; // target is an
  // object
  this.options.offsetTop = options.offsetTop && options.offsetTop ? (
    options.offsetTop === 'function' ? options.offsetTop() : parseInt(options.offsetTop, 0)) : 0; // offset option is
  // an integer number
  // or function to
  // determine that
  // number
  this.options.offsetBottom = options.offsetBottom && options.offsetBottom ? (
    options.offsetBottom === 'function' ? options.offsetBottom() : parseInt(options.offsetBottom, 0)) : null;

  if (!this.element && !(
      this.options.target || this.options.offsetTop || this.options.offsetBottom)) {
    return;
  }
  ...
  this.updatePin = function () {
    if (this.affixed === false && (
        parseInt(this.processOffsetTop(), 0) - parseInt(getScroll()
          .y, 0) < 0)) {
      this.pinTop();
    } else if (this.affixed === true && (
        parseInt(getScroll()
          .y, 0) <= parseInt(this.getPinOffsetTop(), 0))) {
      this.unPinTop()
    }

    if (this.affixedBottom === false && (
        parseInt(this.processOffsetBottom(), 0) - parseInt(getScroll()
          .y, 0) < 0)) {
      this.pinBottom();
    } else if (this.affixedBottom === true && (
        parseInt(getScroll()
          .y, 0) <= parseInt(this.getPinOffsetBottom(), 0))) {
      this.unPinBottom()
    }
  };
...
for (var a = 0, afl = Affixes.length; a < afl; a++) {
  ...
  if (affix && (
      options.offsetTop !== null || options.offsetBottom !== null || options.target !== null)) { //don't do anything unless we have something valid to pin
...
var Alert = function (element) {
  ...
  document.addEventListener('click', this.close, false); //delegate to all alerts, including those inserted later into
  // the DOM
};
...
var Button = function (element, option) {
  ...
  var  changeEvent = (('CustomEvent' in window) && window.dispatchEvent) ? new CustomEvent('bs.button.change') : null;
  ...
  this.toggle = function (e) {
    ...
    if (!label) {
      return;
    } //react if a label or its immediate child is clicked
    ...
    var target = this, //e.currentTarget || e.srcElement; // the button group, the target of the handler function
      ...
      ll = labels.length,
      i = 0, // all the button group buttons
...
var Carousel = function (element, options) {
  ...
  this._slideTo = function (next, e) {
    ...
    //register events
    if ((
        'CustomEvent' in window) && window.dispatchEvent) {
    ...
    if (/\bslide/.test(this.carousel.className) && !(
        isIE && isIE < 10)) {
  ...
  this.indicatorHandler = function (e) {
    ...
    if (target && !/\bactive/.test(target.className) && target.getAttribute('data-slide-to')) {
      ...
      if ((
          active < self.index) || (
          active === self.total - 1 && self.index === 0)) {
        self.direction = 'left'; // next
      } else if ((
          active > self.index) || (
          active === 0 && self.index === self.total - 1)) {
...
var Collapse = function (element, options) {
  ...
  var getOuterHeight = function (el) {
    var s = el && (
        el.currentStyle || window.getComputedStyle(el)), // the getComputedStyle polyfill would do this for us, but we
    ...
    return el.clientHeight + parseInt(btp) + parseInt(mtp) + parseInt(mbp) + parseInt(mte) + parseInt(mbe); //we need
    // an
    // accurate
    // margin
    // value
  ...
  this.getTarget = function () {
    var t = this.btn,
      ...
      .replace('#', ''),
      d = t.getAttribute('data-target') && (
        t.getAttribute('data-target')),
      id = h || (
        d && /#/.test(d)) && d.replace('#', ''),
      cl = (
        d && d.charAt(0) === '.') && d, //the navbar collapse trigger targets a class
    ...
    getWindowWidth = function () {
      var htmlRect = document.documentElement.getBoundingClientRect(),
        fullWindowWidth = window.innerWidth || (
          htmlRect.right - Math.abs(htmlRect.left));
    ...
    setScrollbar = function () {
      ...
      if (self.bodyIsOverflowing) {
        document.body.style.paddingRight = (
          bodyPad + self.scrollbarWidth) + 'px';
      }
    },
    ...
    for (i; i < tgl; i++) {
      triggers[i].addEventListener('click', function (e) {
        ...
        var b = this, // var b = e.target,
          s = b.getAttribute('data-target') && b.getAttribute('data-target')
          .replace('#', '') ||
          b.getAttribute('href') && b.getAttribute('href')
          .replace('#', '');
...
var Popover = function (element, options) {
  ...
  if (!this.content && !this.options.template) {
    return;
  }
  ...
  this.showPopover = function () {
    !/\bin/.test(this.popover.className) && (
      addClass(this.popover, 'in'));
  };
  this.stylePopover = function () {
    var rect = this.link.getBoundingClientRect(),
      scroll = getScroll(), // link rect | window vertical and horizontal scroll
      linkDimensions = {
    ...
    this.popover.className.indexOf(placement) === -1 && (
      this.popover.className = this.popover.className.replace(tipPositions, placement));
  };
  ...
  if (!(
      isIE && isIE < 9)) { // dismiss on window resize
...
var ScrollSpy = function (element, item, options) {
  ...
  this.options.target = options.target ? (
    typeof options.target === 'object' ? options.target : document.querySelector(options.target)) : null;
  ...
  this.tg = this.item.href && document.getElementById(this.item.getAttribute('href')
    .replace('#', ''));
  this.active = false;
  this.topEdge = 0;
  this.bottomEdge = 0;
  ...
  this.topLimit = function () { // the target offset
    if (this.scrollTarget === window) {
      return this.tg.getBoundingClientRect()
        .top + this.scrollOffset() - 5
    } else {
  ...
  this.activate = function () {
    if (this.parent && this.parent.tagName === 'LI' && !/\bactive/.test(this.parent.className)) {
      ...
      if (this.parentParent && this.parentParent.tagName === 'LI' // activate the dropdown as well
        ...
        !/\bactive/.test(this.parentParent.className)) {
  ...
  this.toggle = function () {
    if (this.active === false &&
      ...
      this.activate();
        ...
        this.bottomEdge <= this.scrollOffset() && this.scrollOffset() < this.topEdge)) {
      this.deactivate()
    }
  }
  ...
  // init
  if (this.item.getAttribute('href') && this.item.getAttribute('href')
    .indexOf('#') > -1) {
    //actions
    this.checkEdges();
    this.refresh()
...
//SCROLLSPY DATA API
//=============
var scrollSpyes = document.querySelectorAll('[data-spy="scroll"]'); // mostly is the document.body or a large container
// with many elements having id="not-null-id"
for (var s = 0, ssl = scrollSpyes.length; s < ssl; s++) {
  var spy = scrollSpyes[s],
    options = {};
...
var Tab = function (element, options) {
  ...
  this.handle = function (e) {
    e = e || window.e;
    e.preventDefault();
    var next = self.tab; //the tab we clicked is now the next tab
    var nextContent = document.getElementById(next.getAttribute('href')
      .replace('#', '')); //this is the actual object,
    // the next tab content to
    // activate
...
var Tooltip = function (element, options) {
  ...
  this.options.delay = parseInt(options.delay) || 100;
  this.duration = 150;
  this.options.duration = isIE && isIE < 10 ? 0 : (
    options.duration || this.duration);
  ...
  this.styleTooltip = function () {
    ...
    this.tooltip.className.indexOf(placement) === -1 && (
      this.tooltip.className = this.tooltip.className.replace(tipPositions, placement));
  };
  ...
  this.showTooltip = function () {
    !/\bin/.test(this.tooltip.className) && (
