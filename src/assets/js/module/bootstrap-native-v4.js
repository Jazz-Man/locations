...
var Alert = function (element) {
  ...
  this.close = function () {
    if (alert && element && hasClass(alert, showClass)) {
      ...
      (function () {
        alert && emulateTransitionEnd(alert, transitionEndHandler);
      }())
    }
  };
...
var Carousel = function (element, options) {
  ...
  console.log(indicator);
    var indicators = indicator[getElementsByTagName]("LI");
...
var Modal = function (element, options) { // element can be the modal/triggering button
  ...
  var self = this,
    ...
    fixedItems = getElementsByClassName(doc, 'fixed-top')
    .concat(getElementsByClassName(doc, 'fixed-bottom')),
  ...
  // public methods
  this.toggle = function () {
    if (open && hasClass(modal, showClass)) {
      this.hide();
    } else {
      this.show();
    }
  ...
  this.setContent = function (content) {
    queryElement('.' + component + '-content', modal)
      .innerHTML = content;
  };
...
var Popover = function (element, options) {
  ...
  if (!(stringPopover in element)) { // prevent adding event handlers twice
    if (self[trigger] === hoverEvent) {
      ...
      if (!self[dismissible]) {
        on(element, 'blur', self.hide);
      }
    }

    if (self[dismissible]) {
      on(document, clickEvent, dismissibleHandler);
    }
  ...
  var updateItem = function (index) {
      ...
      if (!isActive && inside) {
        ...
        if (hasClass(item, active)) {
          ...
          isActive = false;
          if (dropdownLink && hasClass(dropdownLink, active) && !getElementsByClassName(item[parentNode], active)
            .length) {
    ...
    updateItems = function () {
      scrollOffset = isWindow ? getScroll()
        .y : element[scrollTop];
